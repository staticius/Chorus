 ...
 * with **primitive int keys**, better scaling properties and
 * generally lower costs.  The use of `int` keys allows for faster
 ...
 *
 *
 * **The main benefit of this class** over using plain [ ] with [Integer] keys is
...
class Int2ObjectNonBlockingMap<TypeV>
@JvmOverloads constructor(initial_sz: Int = MIN_SIZE, opt_for_space: Boolean = true) :
    AbstractMap<Int, TypeV?>(), ConcurrentMap<Int, TypeV?>, Cloneable, Serializable {
    ...
    private class Prime(val _V: Any) {
        companion object {
            fun unbox(V: Any?): Any {
    ...
    // Count of reprobes
    @Transient
    private var _reprobes = ConcurrentAutoIntTable()
    ...
    fun reprobes(): Long {
        val r = _reprobes.get()
        _reprobes = ConcurrentAutoIntTable()
        return r
    }
    ...
    private fun initialize(initial_sz: Int) {
        ...
        _chm = CHM(this, ConcurrentAutoIntTable(), i)
     ...
     * @throws NullPointerException if the specified value is null
     */
    fun put(key: Int, `val`: TypeV): TypeV? {
     ...
     */
    fun putIfAbsent(key: Int, `val`: TypeV): TypeV? {
     ...
     * <tt>null</tt> if there was no mapping for <tt>key</tt>
     */
    override fun remove(key: Int): TypeV? {
     ...
     * @throws NullPointerException if the specified value is null
     */
    fun remove(key: Int, `val`: Any): Boolean {
     ...
     * @throws NullPointerException if the specified value is null
     */
    fun replace(key: Int, `val`: TypeV): TypeV? {
     ...
     * @throws NullPointerException if the specified value is null
     */
    fun replace(key: Int, oldValue: TypeV, newValue: TypeV): Boolean {
        return putIfMatch(key, newValue, oldValue) === oldValue
    }

    private fun putIfMatch(key: Int, newVal: Any, oldVal: Any): TypeV? {
    ...
    override fun clear() {         // Smack a new empty table down
        val newchm = CHM(this, ConcurrentAutoIntTable(), MIN_SIZE_LOG)
    ...
    override fun get(key: Int): TypeV? {
        if (key == NO_KEY) {
            val V = _val_1
    ...
    override fun get(key: Any): TypeV? {
        return if (key is Int) get(key) else null
    }
    ...
    override fun remove(key: Any): TypeV? {
        return if (key is Int) remove(key) else null
    }
    ...
    override fun remove(key: Any, Val: Any): Boolean {
        return (key is Int) && remove(key, Val)
    }
    ...
    override fun containsKey(key: Any): Boolean {
        return (key is Int) && containsKey(key)
    }

    /**
     * Auto-boxing version of [.putIfAbsent].
     */
    override fun putIfAbsent(key: Int, `val`: TypeV): TypeV? {
     ...
     */
    override fun replace(key: Int, Val: TypeV): TypeV? {
     ...
     */
    override fun put(key: Int, `val`: TypeV?): TypeV? {
        return put(key, `val`)
    }
     ...
     * Auto-boxing version of [.replace].
     */
    override fun replace(key: Int, oldValue: TypeV, newValue: TypeV): Boolean {
    ...
    private class CHM(// Back-pointer to top-level structure
        val _nbhml: Int2ObjectNonBlockingMap<*>, // Size in active K,V pairs
        private var _size: ConcurrentAutoIntTable, logsize: Int
    ) : Serializable {
        fun size(): Int {
        ...
        private var _slots: ConcurrentAutoIntTable
        ...
        fun CAS_key(idx: Int, old: Int, key: Int): Boolean {
        ...
        fun putIfMatch(key: Int, putval: Any, expVal: Any?): Any? {
            val hash = hash(key)
            checkNotNull(putval)
        ...
        fun copy_slot(idx: Int): Boolean {
            ...
            while ((_keys[idx].also { key = it }) == NO_KEY) CAS_key(
                idx,
                NO_KEY,
                (idx + _keys.size) /*a non-zero key which hashes here*/
            )
    ...
    private inner class SnapshotV : MutableIterator<TypeV?>, Enumeration<TypeV?> {
        ...
        private var _nextK = 0
        private var _prevK = 0 // Last 2 keys found
        ...
        fun removeKey() {
            checkNotNull(_prevV)
            this@Int2ObjectNonBlockingMap.putIfMatch(_prevK, TOMBSTONE, NO_MATCH_OLD)
            _prevV = null
        }
    ...
    override fun values(): Collection<TypeV> {
        return object : AbstractCollection<TypeV?>() {
            override fun clear() {
                this@Int2ObjectNonBlockingMap.clear()
            }

            override fun size(): Int {
                return this@Int2ObjectNonBlockingMap.size
            }

            override fun contains(v: Any?): Boolean {
                return this@Int2ObjectNonBlockingMap.containsValue(v)
            }
     ...
     * interfaces, generified to the [Integer] class and supporting a
     * **non-auto-boxing** [.nextInt] function.
     */
    inner class IteratorInteger : MutableIterator<Int>, Enumeration<Int>, IntIterator {
        ...
        /**
         * Remove last key returned by [.next] or [.nextInt].
         */
        override fun remove() {
            _ss.removeKey()
        }
        ...
        /**
         * Return the next key as a primitive `int`.
         */
     ...
     * @see .keySet
     */
    fun keys(): Enumeration<Int> {
        return IteratorInteger()
    }

    fun fastKeyIterator(): IteratorInteger {
        return IteratorInteger()
    }
     ...
     * to construction.
     */
    override fun keySet(): Set<Int> {
        return object : AbstractIntSet() {
            override fun clear() {
                this@Int2ObjectNonBlockingMap.clear()
            }
            ...
            override fun contains(k: Int): Boolean {
                return this@Int2ObjectNonBlockingMap.containsKey(k)
            }

            override fun remove(k: Int): Boolean {
                return this@Int2ObjectNonBlockingMap.remove(k) != null
            }

            override fun iterator(): IteratorInteger {
                return IteratorInteger()
            }
        }
    }
     ...
     */
    fun keySetInt(): IntArray {
        val dom = IntArray(size)
        val i: IteratorInteger = keys.iterator() as IteratorInteger
        var j = 0
        while (j < dom.size && i.hasNext()) dom[j++] = i.nextInt()
        return dom
    }
    ...
    // new NBHMLEntry.
    private inner class NBHMLEntry(k: Int, v: TypeV) : IntObjectEntry<TypeV>,
        MutableMap.MutableEntry<Int?, TypeV> {
        override val intKey: Int = k
        ...
        override fun setValue(`val`: TypeV): TypeV {
            ...
            value = `val`
            return put(intKey, `val`)
        }
    }

    private inner class SnapshotE : MutableIterator<IntObjectEntry<TypeV>> {
        val _ss: SnapshotV
        ...
        override fun next(): IntObjectEntry<TypeV> {
            _ss.next()
    ...
    private inner class SnapshotESlow :
        MutableIterator<Map.Entry<Int, TypeV>> {
        val _ss: SnapshotV
        ...
        override fun next(): Map.Entry<Int, TypeV> {
            _ss.next()
            return NBHMLEntry(_ss._prevK, _ss._prevV)
        }
     ...
     * iteration.  The [org.chorus.utils.collection.nb.Int2ObjectNonBlockingMap]
     ...
     */
    @Deprecated("")
    override fun entrySet(): Set<Map.Entry<Int, TypeV>> {
        return object : AbstractSet<Map.Entry<Int?, TypeV>?>() {
            override fun clear() {
                this@Int2ObjectNonBlockingMap.clear()
            }
            ...
            override fun remove(o: Any): Boolean {
                ...
                return this@Int2ObjectNonBlockingMap.remove(o.key, o.value)
            }
            ...
            override fun iterator(): MutableIterator<Map.Entry<Int, TypeV>> {
                return SnapshotESlow()
            }
        }
    }

    fun fastEntrySet(): Set<IntObjectEntry<TypeV>> {
        return object : AbstractSet<IntObjectEntry<TypeV>?>() {
            override fun clear() {
                this@Int2ObjectNonBlockingMap.clear()
            }
            ...
            override fun iterator(): MutableIterator<IntObjectEntry<TypeV>> {
                return SnapshotE()
            }
        }
    }
     ...
     */
    override fun computeIfAbsent(key: Int, mappingFunction: Function<in Int, out TypeV>): TypeV? {
     ...
     * It is not possible to atomically compute a value in a ConcurrentMap without locks.
     */
    override fun computeIfPresent(key: Int, remappingFunction: BiFunction<in Int, in TypeV, out TypeV?>): TypeV? {
     ...
     * It is not possible to atomically compute a value in a ConcurrentMap without locks.
     */
    override fun compute(key: Int, remappingFunction: BiFunction<in Int, in TypeV?, out TypeV?>): TypeV? {
     ...
     * It is not possible to atomically compute a value in a ConcurrentMap without locks.
     */
    override fun merge(key: Int, value: TypeV, remappingFunction: BiFunction<in TypeV, in TypeV, out TypeV?>): TypeV? {
        return super<ConcurrentMap>.merge(key, value, remappingFunction)
    }

    companion object {
        @Serial
        private const val serialVersionUID = 1484063708153648087L
        ...
        private val _IHandler: VarHandle = MethodHandles.arrayElementVarHandle(
            IntArray::class.java
        )
        ...
        init {
            try {
                _chm_handler = MethodHandles.lookup().`in`(
                    Int2ObjectNonBlockingMap::class.java
                )
                    .findVarHandle(Int2ObjectNonBlockingMap::class.java, "_chm", CHM::class.java)
                _val_1_handler = MethodHandles.lookup().`in`(
                    Int2ObjectNonBlockingMap::class.java
                )
                    .findVarHandle(Int2ObjectNonBlockingMap::class.java, "_val_1", Any::class.java)
