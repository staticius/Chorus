...
class EntityAttackedByOwnerSensor(override var period: Int, protected var changeTarget: Boolean) : ISensor {
    override fun sense(entity: EntityMob) {
        if (entity is EntityOwnable) {
            ...
            if (player != null) {
                val current = entity.getMemoryStorage()[CoreMemoryTypes.ENTITY_ATTACKED_BY_OWNER]
                if (!changeTarget) {
                    if (current != null && current.isAlive()) return
                    else entity.getMemoryStorage().clear(CoreMemoryTypes.ENTITY_ATTACKED_BY_OWNER)
                }
                if (player.lastBeAttackEntity != null) {
                    entity.getMemoryStorage()[CoreMemoryTypes.ENTITY_ATTACKING_OWNER] = player.lastBeAttackEntity
                } else if (player.lastAttackEntity != null) {
                    entity.getMemoryStorage()[CoreMemoryTypes.ENTITY_ATTACKED_BY_OWNER] = player.lastAttackEntity
                } else entity.getMemoryStorage().clear(CoreMemoryTypes.ENTITY_ATTACKED_BY_OWNER)
            }
        }
    }
