...
class EntitySalmon(chunk: IChunk?, nbt: CompoundTag) : EntityFish(chunk, nbt) {
    override fun getIdentifier(): String {
        return EntityID.Companion.SALMON
    }
        ...
        if (this.isLarge()) {
            ...
            if (rand == 1) {
                return arrayOf(
                    ...
                    Item.get((if (this.isOnFire) Item.COOKED_SALMON else Item.SALMON))
                )
            }
        } else if (!this.isLarge()) {
            //只有25%获得骨头 来自wiki https://zh.minecraft.wiki/w/%E9%B2%91%E9%B1%BC
            if (rand == 1) {
                return arrayOf(Item.get(Item.BONE), Item.get((if (this.isOnFire) Item.COOKED_SALMON else Item.SALMON)))
            }
        }
        return arrayOf(Item.get((if (this.isOnFire) Item.COOKED_SALMON else Item.SALMON)))
    }
