...
fun Tag.Companion.from(tag: org.chorus_oss.chorus.nbt.tag.Tag<*>): Tag {
    return when (tag) {
        ...
        is org.chorus_oss.chorus.nbt.tag.CompoundTag -> CompoundTag(tag.tags.mapValues { Tag.from(it.value) })
        ...
        is org.chorus_oss.chorus.nbt.tag.ListTag<*> -> ListTag(tag.all.map { Tag.from(it) })
