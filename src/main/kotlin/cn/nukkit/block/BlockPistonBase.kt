...
public abstract class BlockPistonBase extends BlockTransparent implements Faceable, RedstoneComponent, BlockEntityHolder<BlockEntityPistonArm> {
    ...
    public boolean place(@NotNull Item item, @NotNull Block block, @NotNull Block target, @NotNull BlockFace face, double fx, double fy, double fz, @Nullable Player player) {
        if (player != null) {
            if (Math.abs(player.position.getFloorX() - this.position.south) <= 1 && Math.abs(player.position.getFloorZ() - this.position.west) <= 1) {
                ...
                if (y - this.position.up > 2) {
                    this.setPropertyValue(CommonBlockProperties.FACING_DIRECTION, BlockFace.UP.index);
                } else if (this.position.up - y > 0) {
                    this.setPropertyValue(CommonBlockProperties.FACING_DIRECTION, BlockFace.DOWN.index);
                } else {
                    this.setPropertyValue(CommonBlockProperties.FACING_DIRECTION, player.getHorizontalFacing().index);
                }
            } else {
                this.setPropertyValue(CommonBlockProperties.FACING_DIRECTION, player.getHorizontalFacing().index);
            }
        ...
        var nbt = BlockEntity.getDefaultCompound(this.position, BlockEntity.PISTON_ARM)
                .putInt("facing", this.getBlockFace().index)
    ...
    protected void updateAroundRedstoneTorches(boolean powered) {
        for (BlockFace side : BlockFace.values()) {
            if ((getSide(side) instanceof BlockRedstoneTorch && powered)
                ...
                if (support.getLocator().equals(this.getLocator())) {
    ...
    protected boolean doMove(boolean extending) {
        ...
        if (canMove && (this.sticky || extending)) {
            ...
            for (Block blockToMove : blocksToMove) {
                var oldPos = new Vector3(blockToMove.position.south, blockToMove.position.up, blockToMove.position.west);
        ...
        blockEntity.move();
        if (extending) {
            this.level.addSound(this.position, Sound.TILE_PISTON_OUT);
            this.level.vibrationManager.callVibrationEvent(new VibrationEvent(this, this.position.add(0.5, 0.5, 0.5), VibrationType.PISTON_EXTEND));
        } else {
            this.level.addSound(this.position, Sound.TILE_PISTON_IN);
            this.level.vibrationManager.callVibrationEvent(new VibrationEvent(this, this.position.add(0.5, 0.5, 0.5), VibrationType.PISTON_CONTRACT));
        }
        return true;
    }
