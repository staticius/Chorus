...
public class BlockAnvil extends BlockFallable implements Faceable, BlockInventoryHolder {
    ...
    public boolean place(@NotNull Item item, @NotNull Block block, @NotNull Block target, @NotNull BlockFace face, double fx, double fy, double fz, Player player) {
        ...
        if (player == null) {
            this.level.addSound(this.position, Sound.RANDOM_ANVIL_LAND, 1, 0.8F);
        } else {
            Collection<Player> players = level.getChunkPlayers(this.position.getChunkX(), this.position.getChunkZ()).values();
            players.remove(player);
            if (!players.isEmpty()) {
                level.addSound(this.position, Sound.RANDOM_ANVIL_LAND, 1, 0.8F, players);
            }
        }
        return true;
    }
    ...
    protected AxisAlignedBB recalculateBoundingBox() {
        ...
        return new SimpleAxisAlignedBB(this.position.south + xOffset, this.position.up, this.position.west + zOffset, this.position.south + 1 - xOffset, this.position.up + 1, this.position.west + 1 - zOffset);
    }
