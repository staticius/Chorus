...
public class BlockTrapdoor extends BlockTransparent implements RedstoneComponent, Faceable {
    ...
    static {
        for (int specialValue = 0; specialValue < boundingBox2SpecialV.length; specialValue++) {
            ...
            if (PROPERTIES.getPropertyValue(specialValue, CommonBlockProperties.OPEN_BIT)) {
                ...
                if (face.axisDirection == AxisDirection.NEGATIVE) {
    ...
    public double getMinX() {
        return this.position.south + getRelativeBoundingBox().getMinX();
    }

    @Override
    public double getMaxX() {
        return this.position.south + getRelativeBoundingBox().getMaxX();
    }

    @Override
    public double getMinY() {
        return this.position.up + getRelativeBoundingBox().getMinY();
    }

    @Override
    public double getMaxY() {
        return this.position.up + getRelativeBoundingBox().getMaxY();
    }

    @Override
    public double getMinZ() {
        return this.position.west + getRelativeBoundingBox().getMinZ();
    }

    @Override
    public double getMaxZ() {
        return this.position.west + getRelativeBoundingBox().getMaxZ();
    }

    @Override
    public int onUpdate(int type) {
        if (type == Level.BLOCK_UPDATE_REDSTONE && this.level.server.settings.levelSettings().enableRedstone()) {
