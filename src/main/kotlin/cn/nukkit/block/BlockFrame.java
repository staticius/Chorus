...
public class BlockFrame extends BlockTransparent implements BlockEntityHolder<BlockEntityItemFrame>, Faceable {
    ...
    public boolean onActivate(@NotNull Item item, Player player, BlockFace blockFace, float fx, float fy, float fz) {
        ...
        if (itemFrame.getItem().isNull()) {
            ...
            this.level.addLevelEvent(this.position, LevelEventPacket.EVENT_SOUND_ITEMFRAME_ITEM_ADD);
        } else {
            ...
            this.level.addLevelEvent(this.position, LevelEventPacket.EVENT_SOUND_ITEMFRAME_ITEM_ROTATE);
        }
        return true;
    }
    ...
    public boolean place(@NotNull Item item, @NotNull Block block, @NotNull Block target, @NotNull BlockFace face, double fx, double fy, double fz, @Nullable Player player) {
        ...
        this.level.addSound(this.position, Sound.BLOCK_ITEMFRAME_PLACE);
        return true;
    }

    @Override
    public boolean onBreak(Item item) {
        this.level.setBlock(this.position, layer, Block.get(BlockID.AIR), true, true);
        this.level.addLevelEvent(this.position, LevelEventPacket.EVENT_SOUND_ITEMFRAME_BREAK);
        return true;
    }
    ...
    protected AxisAlignedBB recalculateBoundingBox() {
        ...
        return new SimpleAxisAlignedBB(
                aabb[0][0] + this.position.south, aabb[1][0] + this.position.up, aabb[2][0] + this.position.west,
                aabb[0][1] + this.position.south, aabb[1][1] + this.position.up, aabb[2][1] + this.position.west
        );
    }
}