...
public class BlockPointedDripstone extends BlockFallable {
    ...
    protected void setTipBlock(int x, int y, int z, boolean hanging) {
        ...
        this.level.setBlock(x, y, z, this, true, true);
    }
    ...
    protected int getPointedDripStoneLength(int x, int y, int z, boolean hanging) {
        if (hanging) {
            for (int j = y + 1; j < level.getDimensionData().maxHeight; ++j) {
            ...
            for (int j = y - 1; j > level.getDimensionData().minHeight; --j) {
                String blockId = level.getBlockIdAt(x, j, z);
                if (!Objects.equals(blockId, POINTED_DRIPSTONE)) {
                    return y - j - 1;
                }
        ...
        switch (filledWith.getId()) {
            case FLOWING_LAVA -> {
                ...
                if ((cauldron.getCauldronLiquid() == CauldronLiquid.LAVA || cauldron.isEmpty()) && cauldron.getFillLevel() < 6 && nextDouble <= 15.0 / 256.0) {
                    ...
                    if (event.isCancelled())
                        return;
                    cauldron.setCauldronLiquid(event.liquid);
                    cauldron.setFillLevel(cauldron.getFillLevel() + event.liquidLevelIncrement);
                    cauldron.level.setBlock(cauldron.position, cauldron, true, true);
                    this.level.addSound(this.position.add(0.5, 1, 0.5), Sound.CAULDRON_DRIP_LAVA_POINTED_DRIPSTONE);
                }
            }
            case FLOWING_WATER -> {
                nextDouble = rand.nextDouble();
                if ((cauldron.getCauldronLiquid() == CauldronLiquid.WATER || cauldron.isEmpty()) && cauldron.getFillLevel() < 6 && nextDouble <= 45.0 / 256.0) {
                    ...
                    cauldron.setFillLevel(cauldron.getFillLevel() + event.liquidLevelIncrement);
                    cauldron.level.setBlock(cauldron.position, cauldron, true, true);
                    this.level.addSound(this.position.add(0.5, 1, 0.5), Sound.CAULDRON_DRIP_WATER_POINTED_DRIPSTONE);
                }
            }
        }
    }
