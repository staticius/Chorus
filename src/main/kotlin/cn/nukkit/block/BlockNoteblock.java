...
public class BlockNoteblock extends BlockSolid implements RedstoneComponent, BlockEntityHolder<BlockEntityMusic> {
    ...
    public void emitSound(@Nullable Player player) {
        ...
        this.level.vibrationManager.callVibrationEvent(new VibrationEvent(player != null ? player : this, this.add(0.5, 0.5, 0.5).position, VibrationType.BLOCK_CHANGE));

        Instrument instrument = this.getInstrument();

        this.level.addLevelSoundEvent(this.position, LevelSoundEventPacket.SOUND_NOTE, instrument.ordinal() << 8 | this.getStrength());
