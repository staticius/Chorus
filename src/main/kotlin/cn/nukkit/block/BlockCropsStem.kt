...
public abstract class BlockCropsStem extends BlockCrops implements Faceable {
    ...
    public int onUpdate(int type) {
        ...
        if (ThreadLocalRandom.current().nextInt(1, 3) != 1 
                || level.getFullLight(this.position) < MINIMUM_LIGHT_LEVEL) {
        ...
        if (growth < CommonBlockProperties.GROWTH.getMax()) {
            ...
            if (!ev.isCancelled()) {
                this.level.setBlock(this.position, ev.newState, true);
            }
            return Level.BLOCK_UPDATE_RANDOM;
        }
    ...
    public boolean growFruit() {
        ...
        if (side.isAir() && (d.getId().equals(FARMLAND) || d.getId().equals(GRASS_BLOCK) || d.getId().equals(DIRT))) {
            ...
            Server.getInstance().pluginManager.callEvent(ev);
            if (!ev.isCancelled()) {
                this.level.setBlock(side.position, ev.newState, true);
