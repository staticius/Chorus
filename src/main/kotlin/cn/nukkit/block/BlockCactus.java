...
public class BlockCactus extends BlockTransparent implements BlockFlowerPot.FlowerPotBlock, Natural {
    ...
    public double getMaxY() {
        return this.position.up + 0.9375;
    }
    ...
    public int onUpdate(int type) {
        ...
        if (type == Level.BLOCK_UPDATE_RANDOM) {
            ...
            for (int y = 1; y < 3; ++y) {
                Block b = this.level.getBlock(new Vector3(this.position.south, this.position.up + y, this.position.west));
                if (!b.isAir()) { continue; }
                BlockGrowEvent event = new BlockGrowEvent(b, Block.get(BlockID.CACTUS));
                Server.getInstance().pluginManager.callEvent(event);
                if (!event.isCancelled()) {
                    this.level.setBlock(b.position, event.newState, true);
                }
                break;
            }
