...
import org.chorus_oss.chorus.block.BlockWoodenButton
...
class LevelStorageTest {
    ...
    @Order(2)
    @Test
    fun testWritePalette() {
        val byteBuf = ByteBufAllocator.DEFAULT.ioBuffer()
        byteBuf.writeByte(9) //subchunk version 9
        byteBuf.writeByte(2) //layer 2
        byteBuf.writeByte(-4) //section y -4
        val blockStatePalette = Palette(BlockAir.STATE)
        blockStatePalette.writeToStoragePersistent(
            byteBuf
        ) { obj: BlockState -> obj.blockStateTag }
        val data = ByteArray(byteBuf.readableBytes())
        byteBuf.readBytes(data)
        Assertions.assertEquals(4, data[3].toInt()) //Palette Header
        val bytes = Arrays.copyOfRange(data, 4 + 1024, data.size)
        Assertions.assertArrayEquals(byteArrayOf(1, 0, 0, 0), Arrays.copyOfRange(bytes, 0, 4)) //Palette size
        byteBuf.release()
    }

    @Order(3)
    @Test
    fun testLevelDatLoad() {
        Assertions.assertNotNull(levelDBProvider!!.levelData)
        Assertions.assertEquals("Bedrock level", levelDBProvider!!.levelData.name)
    }
    ...
    fun testCloseAndLoadAgain() {
        ...
        var newProvider = LevelDBProvider(level, "src/test/resources/level3")
        for (i in -1..1) {
            for (j in -1..1) {
        ...
        newProvider = LevelDBProvider(level, "src/test/resources/level3")
        for (i in -1..1) {
            for (j in -1..1) {
        ...
        newProvider.close()
        FileUtils.deleteDirectory(File("src/test/resources/level3"))
    }
    ...
    companion object {
        ...
        var TEST_SECTION_DATA: ByteArray = byteArrayOf(
            9,
            1,
            -4,
            6,
